...
namespace Synthesis.Testing.Fields
{
	public class TestHyperlinkField : TestFieldType, IHyperlinkField
	{
		...
		public virtual string RenderedValue
		{
			get
			{
				return string.Format("<a href=\"{0}\" class=\"{1}\" target=\"{2}\" title=\"{3}\">{4}</a>", Href ?? string.Empty, CssClass ?? string.Empty, Target ?? string.Empty, Title ?? string.Empty, Text ?? string.Empty);
			}
		}
